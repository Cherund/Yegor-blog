...
def register():
    ...
    if form.validate_on_submit():
        ...
        name = form.name.data
        checking_email = User.query.filter_by(email=email).first()
        if not checking_email:
            ...
            flash("That email already registered, please login instead.")
...
def show_post(post_id):
    ...
    if form.validate_on_submit():
        if not current_user.is_authenticated:
            flash('Please, log in or register to leave a comment')
            return redirect(url_for('login'))
        new_comment = Comment(
